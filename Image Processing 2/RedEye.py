...
def red_eye(imageFileName, col1, row1, col2, row2):
    #Cretes a copy of the red eye image
    newImage = imageFileName.copy()
    #Loops through each row and column
    for row in range(row1, row2):
        for col in range(col1, col2):
             #Gets pixel at given location
             v = imageFileName.getPixel(col,row)
             #Runs if the red pixel is greater than 150 and is larger than the sum of the blue and green
             if v.red > v.blue+v.green and v.red > 150:
                #Sets the red pixel to an average of blue and green values
                v.red = (v.blue+v.green)//2
             #Places corrected pixel in image
