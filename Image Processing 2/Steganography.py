...
def secret_image(imageFileName, secret):
    #Creates a copy of the image
    newImage = imageFileName.copy()
    numbers = []
    count = 0

    #Breaks secret word into array of binary values
    for i in range(len(secret)):
        res = ''.join(format(j, 'b') for j in bytearray(secret[i], encoding='utf-8'))
        res = res.zfill(8)
        numbers.append(res)

    #Loops through each pixel replacing last 4 binary digits with 0's, or binary of secret word
    for row in range(oldImage.getHeight()):
        for col in range(oldImage.getWidth()):
            v = imageFileName.getPixel(col, row)

            if v.red >= 0:
                v_red = bin(v.red)
                v_red= v_red[2:len(v_red)-4]
                #Places first 4 binary digits of secret word
                if col >= 10 and count < len(numbers):
                    v_red += numbers[count][4:]
                else:
                    v_red += '0000'
                v.red = int(v_red,2)
            ...
            if v.blue >= 0:
                v_blue = bin(v.blue)
                v_blue = v_blue[2:len(v_blue)-4]
                v_blue += '0000'
                v.blue = int(v_blue,2)

            if v.green >= 0:
                v_green = bin(v.green)
                v_green = v_green[2:len(v_green)-4]
                #Places last 4 binary digits of secret word
                if col >= 10 and count < len(numbers):
                    v_green += numbers[count][:-4]
                    count += 1
                else:
                    v_green += '0000'
                v.green = int(v_green,2)
...
def decrypt_image(Image):
    ...
    for row in range(oldImage.getHeight()):
        for col in range(Image.getWidth()):
            ...
            #Looks for pixels where last 4 binary digits aren't 0's
            if len(bin(v.red)) >= 4 or len(bin(v.green)) >= 4:
                g = bin(v.green)[-4:].replace('b','0')
                r = bin(v.green)[-4:].replace('b', '0')
                if (g or r) != '0000':
                    #Combines string letter in binary
                    text += '{:04b}'.format(int(v.green))
                    text += '{:04b}'.format(int(v.red))
                    text = int(text, 2)
                    #Converts binary letter back to character using its ascii value
                    word += chr(text)
                    text = ''
    return word

if __name__ == '__main__':
    win = ImageWin("cImage Demo",800,500) # redEyes1.gif is 395x489
    oldImage = FileImage('redEyes1.gif')

    #Steganography
    newImage = secret_image(oldImage, 'abcdefghijklmnopqrstuvwxyz')
    dec_text = decrypt_image(newImage)
    print(dec_text)
