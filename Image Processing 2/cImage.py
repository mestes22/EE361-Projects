...
def red_eye(imageFileName, col1, row1, col2, row2):
    newImage = imageFileName.copy()
    for row in range(row1, row2):
        for col in range(col1, col2):
             v = imageFileName.getPixel(col,row)
             if v.red > v.blue+v.green and v.red > 100:
...
if __name__ == '__main__':
    ...
    #Redeye
    no_red = red_eye(oldImage, 0, 100, 350, 400)
    no_red.setPosition(no_red.getWidth() + 1, 0)
    oldImage.draw(win)
    no_red.draw(win)
