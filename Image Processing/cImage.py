...
#      class name running around.  We still don't expect p eople to create an AbstractImage but
...
if __name__ == '__main__':
    ...
    for row in range(newImage.getHeight()):
        v = oldImage.getPixel(0, row)
        v.red = 255
        newImage.setPixel(0, row, v)

    for col in range(newImage.getWidth()):
        v = oldImage.getPixel(col, 0)
        v.red = 255
        newImage.setPixel(col, 0, v)

    for row in range(newImage.getHeight()):
        v = oldImage.getPixel(int(newImage.getWidth()-1), row)
        v.red = 255
        newImage.setPixel(int(newImage.getWidth()-1), row, v)

    for col in range(newImage.getWidth()):
        v = oldImage.getPixel(col, int(newImage.getHeight()-1))
        v.red = 255
        newImage.setPixel(col, int(newImage.getHeight()-1), v)

    print(newImage.getWidth()-1)
